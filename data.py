...
#source_word2idx: most comon words mapped to index
# pos_info: location of each word wrt aspect word
# aspect_is: index location of aspect word

#sp_toks: tokenized words in each sentence
#source_words: all words in vocabulary
#target_words: all aspect terms
...
def read_data(fname, source_count, source_word2idx):
    ...
    source_data, source_loc_data, target_data, target_label,og_target_data,og_source_data = list(), list(), list(), list(),list(),list()
    for sentence in root:
        sptoks = en_nlp((sentence.find('text').text).decode('utf-8'))
        if len(sptoks.text.strip()) != 0:
            ...
            for sptok in sptoks:
                ...
                curr_data.append(sptok.text.lower())
            for asp_terms in sentence.iter('aspectTerms'):
                for asp_term in asp_terms.findall('aspectTerm'):
                    if asp_term.get("polarity") == "conflict": continue  # TODO:
                    t_sptoks = en_nlp((asp_term.get('term')).decode('utf-8'))
                    source_data.append(idx)
                    og_source_data.append(curr_data)
                    ...
                    og_target_data.append([sp.text.lower() for sp in t_sptoks])
                    target_label.append(lab)

    print("Read %s aspects from %s" % (len(source_data), fname))
    return source_data, source_loc_data, target_data, target_label, max_sent_len, source_word2idx, og_source_data,og_target_data
