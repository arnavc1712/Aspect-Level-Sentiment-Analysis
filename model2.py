...
class MemN2N(object):
    def __init__(self, config, sess):
        ...
        self.input = tf.placeholder(tf.int32, [self.batch_size, self.mem_size], name="input") # Aspect word/words
        self.time = tf.placeholder(tf.int32, [None, self.mem_size], name="time") # position vectors fo all sentences
        self.target = tf.placeholder(tf.float32, [self.batch_size, 3], name="target") # target variable
        self.context = tf.placeholder(tf.int32, [self.batch_size, self.mem_size], name="context") # all cotext words
    ...
    def build_memory(self):
        ...
        self.BL_W = tf.Variable(tf.random_normal([2 * self.edim, 1], stddev=self.init_std)) # Weights of how important a context word is to an aspect word
        ...
        location_encoding3dim = tf.tile(tf.expand_dims(location_encoding, 2), [1, 1, self.edim])   # Shape: Number_of_sent x max_Sent_len (number of words in sent) x 300 (word embedding size)



        # m_i = sum A_ij * x_ij + T_A_i
        Ain_c = tf.nn.embedding_lookup(self.A, self.context) # Sentences (Batch size) in which words are represented as word vectors
        ...
        ASPin = tf.nn.embedding_lookup(self.ASP, self.input) # same as Ain_c and Bin_c
        ASPinAgg = tf.reduce_mean(ASPin, axis=1) # Shape: Number_of_sent ( Batch size) x edim   Represents sentence vector
        ASPout2dim = tf.reshape(ASPinAgg, [-1, self.edim]) # Shape: Number_of_sent ( Batch size) x edim
        self.hid.append(ASPout2dim) # List that contains a chunk of a batch size, which contains mean of all the word vectors in each sentence
        ...
        for h in range(self.nhop):
            ...
            P = tf.nn.softmax(g_2dim) # alpha

            probs3dim = tf.reshape(P, [-1, 1, self.mem_size])
            Bout = tf.matmul(probs3dim, Bin)
            Bout2dim = tf.reshape(Bout, [-1, self.edim]) # vec

            Cout = tf.matmul(self.hid[-1], self.C) #linear layer
            Dout = tf.add(Cout, Bout2dim) # sum of attention layer and linear layer
            ...
            if self.lindim == self.edim:
                ...
                F = tf.slice(Dout, [0, 0], [self.batch_size, self.lindim]) # Shape: takes out a Batch_size x lindim vector from Batch_Size x edim
    ...
    def build_model(self):
        ...
        # self.validation_summary = tf.summary.merge_all()
    ...
    def train(self, data):
        source_data, source_loc_data, target_data, target_label, max_sent_len,source_word2idx,og_source_data,og_target_data = data
    ...
    def test(self, data):
        ...
        for i in range(N):
            ...
            # print "Input; {}".format(x[:15])
            
            # print "Og source data; {}".format(og_source_data[:15])
            # print "Target data; {}".format(target[:15])
            # print "Og target data; {}".format(og_target_data[:15])
            # print "Predictions; {}".format(predictions[:15])
            # print "Target label; {}".format(target_label[:15])
            # print "Raw labels; {}".format(raw_labels[:15])
    ...
    def run(self, train_data, test_data):
        ...
        # saver.restore(self.sess,'./models/model-15-9.ckpt-14')
        test_loss, test_acc = self.test(test_data)
        print('test_loss=%.2f;test-acc=%.2f;' % (test_loss, test_acc))
